...
class Consents {
   ...
   * @throws {XcooBeeError}
   */
  async listConsents(filters = {}, config = null) {
    this._assertValidState();

    const fetchPage = async (apiCfg, params) => {
      const { apiKey, apiSecret, apiUrlRoot } = apiCfg;
      const { after, limit, filters: consentFilters } = params;
      const apiAccessToken = await this._.apiAccessTokenCache.get(apiUrlRoot, apiKey, apiSecret);
      const user = await this._.usersCache.get(apiUrlRoot, apiKey, apiSecret);
      const userCursor = user.cursor;
      const consentsPage = await ConsentsApi.listConsents(apiUrlRoot, apiAccessToken, userCursor, consentFilters, after, limit);
      return consentsPage;
    };
    const apiCfg = SdkUtils.resolveApiCfg(config, this._.config);

    return SdkUtils.startPaging(fetchPage, apiCfg, { filters });
  }
   ...
   *
   * @async
   * @param {CampaignRef} [campaignRef] - The reference of the campaign to fetch.
   * @param {Config} [config] - If specified, the configuration to use instead of the
   *   default.
   *
   * @returns {Promise<SuccessResponse>} - The response.
   * @property {number} code - The response status code.
   * @property {Error} [error] - The response error if status is not successful.
   * @property {string} [error.message] - The error message.
   * @property {string} request_id - The ID of the request generated by the XcooBee
   *   system.
   * @property {Object} [result] - The result of the response if status is successful.
   * @property {string} result - The campaign id.
   *
   * @throws {XcooBeeError}
   */
  async getCampaignIdByRef(campaignRef, config = null) {
    this._assertValidState();
    const apiCfg = SdkUtils.resolveApiCfg(config, this._.config);
    const { apiKey, apiSecret, apiUrlRoot } = apiCfg;
    ...
    try {
      const apiAccessToken = await this._.apiAccessTokenCache.get(apiUrlRoot, apiKey, apiSecret);
      const result = await CampaignApi.getCampaignIdByRef(apiUrlRoot, apiAccessToken, campaignRef);
      const response = new SuccessResponse(result);
      return response;
    } catch (err) {
      throw new ErrorResponse(400, err);
   ...
   *
   * @async
   * @param {CampaignRef} campaignRef - The reference of the campaign that consents will be shared to.
   * @param {string} [campaignId] - The campaign id, that consents will be shared from
   * @param {Array} [consentIds] - List of consent ids, that will be shared
   * @param {Config} [config] - If specified, the configuration to use instead of the
   *   default.
   *
   * @returns {Promise<SuccessResponse>} - The response.
   ...
   * @throws {XcooBeeError}
   */
  async shareConsents(campaignRef, campaignId = null, consentIds = [], config = null) {
    this._assertValidState();
    const apiCfg = SdkUtils.resolveApiCfg(config, this._.config);
    const { apiKey, apiSecret, apiUrlRoot } = apiCfg;
   ...
   *   system.
   * @property {Object} [result] - The result of the response if status is successful.
   * @property {string} result - will contain true.
   *
   * @throws {XcooBeeError}
   */
