...
class System {
  ...
  /**
   * Delete event by provided ids.
   *
   * @async
   * @param {number[]} eventIds - Ids of events to delete.
   * @param {Config} [config] - The configuration to use instead of the default.
   *
   * @returns {Promise<SuccessResponse, ErrorResponse>}
   * @property {number} code - The response status code.
   ...
   *   system.
   * @property {Object} [result] - The result of the response if status is successful.
   * @property {Event} result - Deleted events
   *
   * @throws {XcooBeeError}
   */
  async deleteEvents(eventIds, config = null) {
    this._assertValidState();
    ...
    try {
      ...
      const result = await EventsApi.deleteEvents(apiUrlRoot, apiAccessToken, eventIds);
