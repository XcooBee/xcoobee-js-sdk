...
- [System API](#system-api)
    - [ping](#pingconfig)
    - [addEventSubscription](#addeventsubscriptionevents-campaignid-config)
...
The webhook system uses HTTPS `POST` calls to send information to you in response to event in XcooBee processing. You subscribe to events by calling the [`addEventSubscription()`](#add-event-subscription) method.
...
This is one of the simpler parts. As you subscribe to events using the [`addEventSubscription()`](#add-event-subscription) you also supply the JS function that you wish to invoke when such an event is returned as function argument. See the function definition for more information.
